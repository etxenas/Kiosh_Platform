/**
 * Setup Hyrtoaletter schema via REST Tooling API (pålitligare än SOAP).
 * Körs med: node src/setup-schema.mjs
 */
...
// Bygg Tooling API CustomField från vårt format
function buildFieldPayload(f) {
  ...
  if (f.defaultValue !== undefined) md.defaultValue = f.defaultValue;
  if (f.referenceTo) {
  ...
  // Metadata måste wrappas inne i CustomField
  return { FullName: f.fullName, Metadata: md };
}

async function main() {
  console.log('🔧 Building Hyrtoaletter fields via Tooling API...\n');
  ...
  for (const f of FIELDS) {
    ...
    try {
      ...
      const msg = e.message || '';
      if (msg.includes('Duplicate') || msg.includes('already exists') || msg.includes('FIELD_INTEGRITY_EXCEPTION')) {
        skipped++;
        console.log(`  ⏭️  ${f.fullName} (exists)`);
      } else {
        failed++;
        const short = msg.slice(0, 150).replace(/\n/g, ' ');
  ...
  if (failed === 0) console.log('✅ Schema complete!');
}
